...
class Arch:
    ...
    def comp_unrolled_model(self, x, trf, target, eta, optimizer, pos_weight):
        """
        loss on train set and then update w_pi, not-in-place
        :param optimizer: optimizer of theta, not optimizer of alpha
        """
        # forward to get loss
        loss = self.model.loss(x, trf, target, pos_weight)
    ...
    def step(self, x_train, trf_train, target_train, x_valid, trf_valid, target_valid, eta, optimizer, pos_weight, unrolled):
        """
        ...
        if unrolled:
            self.backward_step_unrolled(x_train, trf_train, target_train, x_valid, trf_valid, target_valid, eta, optimizer, pos_weight)
        else:
            # directly optimize alpha on w, instead of w_pi
            self.backward_step(x_valid, trf_valid, target_valid, pos_weight)

        self.optimizer.step()

    def backward_step(self, x_valid, trf_valid, target_valid, pos_weight):
        """
        simply train on validate set and backward
        """
        loss = self.model.loss(x_valid, trf_valid, target_valid, pos_weight)
        # both alpha and theta require grad but only alpha optimizer will
        # step in current phase.
        loss.backward()

    def backward_step_unrolled(self, x_train, trf_train, target_train, x_valid, trf_valid, target_valid, eta, optimizer, pos_weight):
        """
        ...
        unrolled_model = self.comp_unrolled_model(x_train, trf_train, target_train, eta, optimizer, pos_weight)
        # calculate loss on theta_pi
        unrolled_loss = unrolled_model.loss(x_valid, trf_valid, target_valid, pos_weight)
        ...
        implicit_grads = self.hessian_vector_product(vector, x_train, trf_train, target_train, pos_weight)
    ...
    def hessian_vector_product(self, vector, x, trf, target, pos_weight, r=1e-2):
        """
